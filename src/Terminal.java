...
public class Terminal {
  ...
  private void mkdir() {
    ...
    for(String arg : args) {
      ...
      try {
        if (!Files.exists(resolvedAbsolute)) {
          Files.createDirectory(resolvedAbsolute);
  ...
  private void rmdir() {
    ...
    try {
      if (Objects.equals(arg, "*")) {
        ...
        Path parentDir = currPath.getParent();
        Files.walk(currPath)
      ...
      if (parentDir != null) {
        ...
        Path basepath = Paths.get(System.getProperty("user.dir"));
      Path resolvedAbsolute = basepath.resolve(currPath);
        DirectoryStream<Path> dirStream = Files.newDirectoryStream(resolvedAbsolute);
        if (Files.isDirectory(resolvedAbsolute) && !dirStream.iterator().hasNext()) {
          Files.delete(resolvedAbsolute);
          System.out.println(resolvedAbsolute + " deleted");
        } else {
...
//            copying the directories here
  ...
  private void rm() {
    ...
    try {
      Files.delete(resolvedAbsolute);
      System.out.println("file "+ resolvedAbsolute + " deleted");
    } catch (InvalidPathException | IOException e) {
      System.out.println("Invalid path "+ resolvedAbsolute);
    }
  }
  private void cat() {
    String arg = parser.getArgs()[0];
    ...
    Path basepath = Paths.get(System.getProperty("user.dir"));
      Path resolvedAbsolute = basepath.resolve(path);
    try (BufferedReader reader = new BufferedReader(new FileReader(resolvedAbsolute.toString()))) {
      String line;
    ...
    if(parser.getArgs().length>1){
     ...
     path = Paths.get(arg);
      ...
      resolvedAbsolute = basepath.resolve(path);
    try (BufferedReader reader = new BufferedReader(new FileReader(resolvedAbsolute.toString()))) {
      String line;
      while ((line = reader.readLine()) != null) {
          System.out.println(line);
